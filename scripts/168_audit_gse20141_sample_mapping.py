#!/usr/bin/env python3
"""Audit GSE20141 GEO accession joins without reading expression values."""
...
def count_labels(rows: list[dict[str, str]]) -> tuple[int, int]:
    pd = sum(1 for row in rows if row.get("label__pd_vs_control") == "1")
    control = sum(1 for row in rows if row.get("label__pd_vs_control") == "0")
    return pd, control
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit GSE20141 series-matrix-to-metadata sample mapping.")
    parser.add_argument("--series-matrix", type=Path, default=Path("data/raw/external/gse20141_pd_snpc_lcm/GSE20141_series_matrix.txt.gz"))
    parser.add_argument("--metadata", type=Path, default=Path("results/tables/phase34_gse20141_pd_snpc_lcm_sample_metadata.tsv"))
    parser.add_argument("--output", type=Path, default=Path("results/tables/phase35_gse20141_sample_mapping_audit.tsv"))
    parser.add_argument("--preview-output", type=Path, default=Path("results/reports/phase35_gse20141_sample_mapping_preview.txt"))
    parser.add_argument("--log-file", type=Path, default=Path("results/logs/168_gse20141_sample_mapping_audit.log"))
    ...
    logging.info("Best GSE20141 join key=%s", best_key)
    return 0
