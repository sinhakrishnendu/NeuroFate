#!/usr/bin/env python3
"""Run robust sample-level GSE243639 cell-type-aware PD validation."""
...
def repeated_split_predictions(X: np.ndarray, y: np.ndarray, model_name: str, seed: int, repeats: int) -> tuple[list[dict[str, float]], list[dict[str, str]]]:
...
def leave_one_out_predictions(X: np.ndarray, y: np.ndarray, seed: int) -> tuple[dict[str, float], list[dict[str, str]]]:
    ...
    loo = LeaveOneOut()
    for split_id, (train_idx, test_idx) in enumerate(loo.split(X), start=1):
...
def reliability(observed_auroc: float, auroc_sd: float, empirical_p: float) -> str:
...
def feature_importance(rows: list[dict[str, str]], features: list[str], X: np.ndarray, y: np.ndarray, seed: int) -> list[dict[str, str]]:
    model = random_forest_model(seed)
    model.fit(X, y)
    estimator = model.named_steps["model"]
    importances = getattr(estimator, "feature_importances_", np.zeros(len(features)))
...
def summarize_metrics(
    ...
    return {
        ...
        "reliability_flag": reliability(observed, auroc_sd, empirical_p) if model_name == "logistic_regression" else "supporting_baseline_only",
        "notes": "Sample-level cell-type-aware PD/control validation; no clinical interpretation.",
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run robust GSE243639 cell-type-aware PD validation.")
    parser.add_argument("--features", type=Path, default=Path("results/tables/phase17_gse243639_celltype_feature_table.tsv"))
    parser.add_argument("--metrics-output", type=Path, default=Path("results/tables/phase17_gse243639_celltype_validation_metrics.tsv"))
    parser.add_argument("--predictions-output", type=Path, default=Path("results/tables/phase17_gse243639_celltype_predictions.tsv"))
    parser.add_argument("--importance-output", type=Path, default=Path("results/tables/phase17_gse243639_celltype_feature_importance.tsv"))
    parser.add_argument("--log-file", type=Path, default=Path("results/logs/86_run_gse243639_celltype_pd_validation.log"))
...
def main() -> int:
    ...
    if len(rows) < 8 or len(counts) < 2 or min(counts.values()) < 2 or not features:
        raise RuntimeError("GSE243639 cell-type validation requires sample-level rows, two classes, and feature columns.")
    ...
    metric_rows = [
        summarize_metrics("logistic_regression", "repeated_stratified_split", logistic_metrics, len(rows), counts, len(features), empirical_p, null_auroc, ci_low, ci_high),
        summarize_metrics("logistic_regression", "leave_one_out", [loo_metrics], len(rows), counts, len(features), empirical_p, null_auroc, ci_low, ci_high),
        summarize_metrics("random_forest_baseline", "repeated_stratified_split", rf_metrics, len(rows), counts, len(features)),
    ]
    prediction_rows = [*split_predictions, *rf_predictions, *loo_predictions]
    importance_rows = feature_importance(rows, features, X, y, args.seed + 1600)
    metric_fields = [
        "model",
        "validation_mode",
        "task_id",
    ...
    write_tsv(args.predictions_output, prediction_rows, ["model", "validation_mode", "split_id", "row_index", "true_label", "predicted_probability", "predicted_label"])
    write_tsv(args.importance_output, importance_rows, ["rank", "feature", "importance", "model"])
    logging.info("Phase 17 validation completed on sample-level feature table only.")
    return 0
